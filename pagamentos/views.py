...
def finalizar_compra(request):
    if request.method == 'POST':
        ...
        if pagamento_sucesso:
            # Criar a encomenda na app encomendas
            encomenda = Encomenda.objects.create(
                user=request.user,
                data=datetime.now(),
                valor=carrinho_total,
                morada=morada  # Salvar a morada associada à encomenda
            )
            ...
            # Preparar os dados a passar para a template
            compra = {
            ...
            # Passa os dados para a página de sucesso
